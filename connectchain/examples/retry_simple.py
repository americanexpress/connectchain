...
prompt = PromptTemplate(
    ...
    template="Tell me about the a biggest {adjective} animal in the world.",
)

try:
    failed_chain = prompt | RunnableLambda(lambda x: simulated_failure(x)) | StrOutputParser()
    res = failed_chain.invoke(EXAMPLE_INPUT)
except Exception as e:
    print(f"Failed to invoke the model: {e}")

chain = prompt | LCELRetry(RunnableLambda(lambda x: simulated_failure(x))) | StrOutputParser()
