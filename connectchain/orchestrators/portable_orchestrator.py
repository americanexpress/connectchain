...
class PortableOrchestrator:
    ...
    def __init__(self, chain: connectchain.chains.ValidLLMChain, **kvargs):
    ...
    @staticmethod
    def from_prompt_template(prompt_template: str, input_variables: [str], **kwargs):
        """Method to build a PortableOrchestrator instance"""
        index = kwargs.get('index')
        llm = model(index or '1')

        prompt_sanitizer = kwargs['prompt_sanitizer'] \
            if kwargs.get('prompt_sanitizer') \
            else lambda x: x
        ...
        chain = connectchain.chains.ValidLLMChain(llm=llm, prompt=prompt)
