...
class TestPortableOrchestrator(unittest.TestCase):
    ...
    def setUp(self) -> None:
        self.from_env_patcher = patch('connectchain.utils.Config.from_env', return_value=get_mock_config())
        self.mock_from_env = self.from_env_patcher.start()
        self.chat_openai_patcher = patch('connectchain.lcel.model.ChatOpenAI', return_value=Mock(ChatOpenAI))
    ...
    def test_build_with_missing_config(self, *args): # pylint: disable=unused-argument
        ...
        with self.assertRaisesRegex(BaseException, 'Model config at index "gpt5" is not defined') as _:
