...
class TestTokenUtil(TestCase):
    ...
    @patch(
        ...
        return_value=get_mock_config({"models": {}}),
    )
    def test_token_from_env_no_model_at_index(self, *args):
        """Test for missing id_key environment variable"""
        with self.assertRaisesRegex(
            UtilException, 'Model config at index "other" is not defined'
        ) as _:
            get_token_from_env("other")

    @patch(
        "connectchain.utils.token_util.Config.from_env",
        return_value=get_mock_config({"models": {"other": {"eas": {"id_key": "missing_key"}}}}),
    )
    def test_token_from_env_no_id_key(self, *args):
        """Test for missing id_key environment variable"""
        with self.assertRaisesRegex(
            UtilException, 'Environment variable id key "missing_key" not set for model index other'
        ) as _:
            get_token_from_env("other")
    ...
    def test_token_from_env_no_secret_key(self, *args):
        ...
        with self.assertRaisesRegex(
            UtilException,
            'Environment variable secret key "missing_key" not set for model index other',
        ) as _:
            get_token_from_env("other")
